...
#print(api_for_weather('Vadodara'))
...
def work(request):
    if request.method=='POST':
        ...
        #print(prod_area.objects.filter(crop=crop,district=area).count())
    ...
    # handling the imports exports and production factors
            ...
            return render(request,'cultivo_main/parallax.html',{'first':first_value,'second':sec_values,'third':third_values, 'temp_det':temp_det,'final':final_dict,'soil':soil_info,'finalout_main':final_main,'finalout_dec':final_dec,'ff':fin_dict,'firstname':firstname, 'lastname':lastname, 'email':email, 'phone':phone})
        else:
            return render(request,'cultivo_main/new.html',{'crop':crop,'firstname':firstname, 'lastname':lastname, 'email':email, 'phone':phone})
